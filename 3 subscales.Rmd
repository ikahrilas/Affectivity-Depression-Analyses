...
# for deriving reliability coefficients
...
items <- list(select(sad_data, anticipating_items), 
              select(sad_data, moment_items), 
              select(sad_data, reminiscing_items)
)
...
Now, derive the total for the PHQ-9
...
```{r calculate Macdonald's omega for subscales}

ci.reliability(data = sad_data %>% select(contains("pswq")), type = "omega")
```

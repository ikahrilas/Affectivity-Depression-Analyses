...
mean_pa <- sad_data_severity %>% 
  split(.$severity) %>% 
  map_df(~ {
    sprintf("%.2f", round(mean(.x$masq_pa), digits = 2))
  }) %>% 
...
sd_pa <- sad_data_severity %>% 
  split(.$severity) %>% 
  map_df(~ { 
    sprintf("%.2f", round(sd(.x$masq_pa), digits = 2))
    }) %>% 
  mutate(header = "PA SD") %>% 
  select(header, everything())
...
mean_na <- sad_data_severity %>% 
  split(.$severity) %>% 
  map_df(~ {
    sprintf("%.2f", round(mean(.x$masq_na), digits = 2))
  }) %>% 
...
sd_na <- sad_data_severity %>% 
  split(.$severity) %>% 
  map_df(~ { 
    sprintf("%.2f", round(sd(.x$masq_na), digits = 2))
    }) %>% 
  mutate(header = "NA SD") %>% 
  select(header, everything())
...
mean_stm <- sad_data_severity %>% 
  split(.$severity) %>% 
  map_df(~ {
    sprintf("%.2f", round(mean(.x$savoring_moment), digits = 2))
  }) %>% 
...
sd_stm <- sad_data_severity %>% 
  split(.$severity) %>% 
  map_df(~ { 
    sprintf("%.2f", round(sd(.x$savoring_moment), digits = 2))
    }) %>% 

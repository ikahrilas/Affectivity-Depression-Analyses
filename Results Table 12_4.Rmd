...
# F statistic value
f_ant <- sprintf("%.1f", summary(ant_model)$fstatistic[[1]])
f_mom <- sprintf("%.1f", summary(mom_model)$fstatistic[[1]])
f_rem <- sprintf("%.1f", summary(rem_model)$fstatistic[[1]])
f_dep <- sprintf("%.1f", summary(dep_model)$fstatistic[[1]])

# R-squared value
rsquare_ant <- sprintf("%.3f", summary(ant_model)$r.squared)
rsquare_mom <- sprintf("%.3f", summary(mom_model)$r.squared)
rsquare_rem <- sprintf("%.3f", summary(rem_model)$r.squared)
rsquare_dep <- sprintf("%.3f", summary(dep_model)$r.squared)
```
...
#```{r creating functions to add text padding so that decimal points align}
# adds padding to coefficients column
coef_pad <- function(x) {
  map_chr(x$Coef., ~
          if_else(str_detect(.x, "-"), 
                  .x, 
                  str_pad(.x, width = 6, side = "left")
                  )
        )
}
# adds padding to p-value column
p_pad <- function(x) {
  map_chr(x$p, ~
          if_else(str_detect(.x, "0\\."),
                  str_pad(.x, width = 6, side = "left"),
                  .x
                  )
        )
...
```{r replace some p values as per Fred's request}
results_m1[10, 3] <- "<.006"
results_m3[10, 3] <- "<.088"
```

#```{r add padding to columns using defined functions}
results_m1$Coef. <- coef_pad(results_m1)
results_m1$p <- p_pad(results_m1)
results_m2$Coef. <- coef_pad(results_m2)
results_m2$p <- p_pad(results_m2)
results_m3$Coef. <- coef_pad(results_m3)
results_m3$p <- p_pad(results_m3)
results_dep$Coef. <- coef_pad(results_dep)
results_dep$p <- p_pad(results_dep)
#```
...
```{r latex table}
kable(results_total, "latex", escape = FALSE, booktabs = TRUE, align = c("l", rep(c("c", "r", "c", "c"), 4), "c", "r", "c"), linesep = "", caption = "Unstandardized path coefficients from structural equation model using positive and negative affectivity to predict depression with savoring subscales as mediators ($N$ = 1,680)") %>%
  ...
  footnote(general = "$SE$ = Standard Error, PA = Positive Affectivity, NA = Negative Affectivity, ANT = Anticipating, MOM = Savoring the Moment, REM = Reminiscing, DEP = Depression, WOR = Worry, ANX = Anxious Arousal.", threeparttable = TRUE) %>%
...
```{r latex table}
kable(results_total, "latex", escape = FALSE, booktabs = TRUE, align = c("l", rep("r", times = 19)), linesep = "", caption = "Unstandardized path coefficients from structural equation model using positive and negative affectivity to predict depression with savoring subscales as mediators ($N$ = 1,680)") %>%
  kable_styling(latex_options = c("hold_position", "scale_down")) %>% 
  add_header_above(c(" ", " ", "$M_1$(ANT)" = 3, " ", "$M_2$(MOM)" = 3, " ", "$M_3$(REM)" = 3, " ", "$Y(DEP)$" = 3, " ", "Indirect Effects" = 2), escape = FALSE) %>% 
  add_indent(c(2:4, 6:8)) %>% 
  footnote(general = "$SE$ = Standard Error, PA = Positive Affectivity, NA = Negative Affectivity, ANT = Anticipating, MOM = Savoring the Moment, REM = Reminiscing, DEP = Depression, WOR = Worry, ANX = Anxious Arousal.", threeparttable = TRUE) %>%
